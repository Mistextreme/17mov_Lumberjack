...
function ShowCustomPanel(title, subttitle, content, img, confirmBtn, closeBtn) {
    $(".header .bgText").text(title)
...
async function ShowNotification(type, msg) {
    ...
    if (type == "wrong") {
        ...
        if (msg.length > 135) {
            notificationTemplate = `<div style="transform: translateX(106%)" class="notification " id="notify${thisId}"><div class="flexBox"><div class="text longNotify"><div class="title">NOTIFICATION</div><div class="content">${msg}</div></div></div><div class="progressbar"><div class="track"><div class="rdyTrack" id="track${thisId}"></div></div></div></div>`
        } else {
            notificationTemplate = `<div style="transform: translateX(106%)" class="notification" id="notify${thisId}"><div class="flexBox"><div class="icon"><img src="assets/check.svg"></div><div class="text shortNotify"><div class="title">NOTIFICATION</div><div class="content">${msg}</div></div></div><div class="progressbar"><div class="track"><div class="rdyTrack" id="track${thisId}"></div></div></div></div>`
...
window.addEventListener('message', function (event) {
    ...
    if (action == "addNewMember") {
        ...
        ShowCustomPanel(event.data.title, event.data.subTitle, event.data.content, event.data.img, event.data.confirmBtn, event.data.closeBtn)
    } else if (action == "Init") {
...
JS17Movement.PlaySound = (id, audioFile, volume, maxDistance, x, y, z) => {
	...
	fetch(`sounds/${audioFile}`)
		...
		.then((audioBuffer) => {
			...
			JS17Movement.Sounds[id] = { panner, x, y, z };
			source.start();
		});
	...
	source.onended = () => {
		...
		$.post(`https://${GetParentResourceName()}/soundsEnd`, JSON.stringify({ id: id }));
	};
};
...
JS17Movement.UpdateSounds = (x, y, z, h) => {
	...
	Object.values(JS17Movement.Sounds).forEach((data) => {
		...
		data.panner.setPosition(pannerX, pannerZ, -pannerY);
		data.panner.setOrientation(-orX, 0, orY);
	});
};
...
window.addEventListener('message', function (event) {
    ...
    } else if (data.sound == "updateSounds") {
		JS17Movement.UpdateSounds(data.x, data.y, data.z, data.h);
    } else if (data.sound == "updateSoundCoords") {
